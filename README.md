...
| `/turmas/nova` · `/turmas/:id/editar` | **Nova / Editar turma** | Formulário reativo com dias, modalidade, cor, disciplina, horários e a config de **Pontuação & Gamificação** (liga/desliga, nome da pontuação, rótulos dos botões, ranking on/off). Um *toggle* **Ensino Regular** troca a turma para o modo escola: seleção de **instituição + nível + ano/série** e uma **grade de alocação** (dia × horário) — os dias da semana passam a vir das alocações. Editar **reprojeta** a agenda. Criar respeita a **cota do plano** (com *upsell* ao estourar). |
...
| `/configuracoes` | **Hub de Configurações** | Índice da conta: cabeçalho com **avatar + nome** do professor e um menu para **Meu Perfil**, **Instituições**, **Meu Plano** e a seção **Legal** (**Termos de Uso** e **Política de Privacidade**). |
...
| `/configuracoes/instituicoes` | **Instituições** | Escolas do **ensino regular**: cadastro/edição em modal (início/fim do dia, duração da aula, intervalo) com **prévia ao vivo da grade** (1º Horário, Intervalo, …) gerada pelo backend. |
