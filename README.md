...
| `/turmas` | **Minhas Turmas** | Centro do fluxo do **ensino regular**. Conta virgem: um **card-botão "Criar Instituição de Ensino"** (passo obrigatório antes das turmas). Com dados: as turmas ficam **agrupadas em cards por instituição** (expandem para listar as turmas + "Nova turma nesta escola"), e o legado cai num bloco **"Turmas sem uma instituição de ensino"**. Instituições são criadas/editadas aqui, em modal com **prévia ao vivo da grade**. |
| `/turmas/nova` · `/turmas/:id/editar` | **Nova / Editar turma** | Formulário reativo com modalidade, cor, disciplina e a config de **Pontuação & Gamificação** (liga/desliga, nome da pontuação, rótulos dos botões, ranking on/off). A **Modalidade** é **Ensino Regular** (escola) ou **Módulo fechado**: em *Ensino Regular*, escolhe-se **instituição + nível + ano/série** e a **grade de alocação** (dia × horário), com os dias derivados das alocações; em *Módulo*, define-se dias/horários/total de aulas. Editar **reprojeta** a agenda. Criar respeita a **cota do plano** (com *upsell* ao estourar). |
